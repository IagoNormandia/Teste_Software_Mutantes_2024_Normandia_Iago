...
class TestRequests:
    ...
    @pytest.mark.parametrize(
        "auth_header_key", ["authorization", "AUTHORIZATION", "AuThOrIzAtIoN"]
    )
    def test_auth_header_is_stripped_on_cross_domain_redirect_case_insensitive(self, httpbin, auth_header_key):
        """
        Confirma que o cabeçalho de autorização é removido durante um redirecionamento
        para um domínio DIFERENTE, independentemente da capitalização.
        """
        # 1. Simular um domínio diferente.
        #    Substituímos '127.0.0.1' por 'localhost'. Para a biblioteca, são hosts diferentes.
        base_url = httpbin("/get")
        redirect_url_cross_domain = base_url.replace("127.0.0.1", "localhost")

        # 2. Configurar o redirecionamento para o nosso URL "cross-domain".
        url = httpbin(f"/redirect-to?url={redirect_url_cross_domain}")
        headers = {auth_header_key: "my-secret-token"}

        # 3. Executar a requisição.
        s = requests.Session()
        response = s.get(url, headers=headers, allow_redirects=True)

        # 4. Verificar se o cabeçalho foi removido na requisição final.
        assert auth_header_key not in response.request.headers
    ...
    def test_auth_header_is_stripped_on_cross_domain_redirect_case_insensitive(self, httpbin, auth_header_key):
        ...
        Confirma que o cabeçalho de autorização é removido durante um redirecionamento
        para um domínio DIFERENTE, independentemente da capitalização.
        Mata o mutante que altera a capitalização de 'Authorization'.
        """
        # 1. Simular um domínio diferente.
        #    Substituímos '127.0.0.1' por 'localhost'. Para a biblioteca, são hosts diferentes.
        base_url = httpbin("/get")
    ...
    def test_session_convenience_methods_use_uppercase_verb(self, httpbin, method_name, expected_verb):
        """
        Confirma que os métodos de conveniência da Sessão (como .get(), .post())
        usam a forma canónica e em maiúsculas do verbo HTTP.
        Mata mutantes que alteram a capitalização do verbo.
        """
        # 1. Obter o método da sessão a ser testado (ex: s.get)
        s = requests.Session()
        method_to_test = getattr(s, method_name)

        # 2. Fazer uma chamada usando o método de conveniência
        response = method_to_test(httpbin("/get"))

        # 3. Verificar (Assert) que o verbo na requisição preparada está em maiúsculas
        assert response.request.method == expected_verb
    ...
    def test_iter_lines_default_decode_unicode_is_false(self):
        ...
        bytes brutos (decode_unicode=False).
        Mata o mutante que altera o valor padrão de decode_unicode para True.
        """
        # 1. Preparar dados de bytes que não são UTF-8 válidos
        text = "français"
        raw_content = text.encode("latin-1")

        # 2. Criar uma resposta simulada com um corpo de "raw" bytes
        response = Response()
        response.raw = io.BytesIO(raw_content)
        response.encoding = "latin-1"  # Definir o encoding para ser consistente

        # 3. Chamar iter_lines com os argumentos padrão
        lines_iterator = response.iter_lines()
        first_line = next(lines_iterator)
...
class TestPreparingURLs:
    ...
    def test_request_prepare_passes_json_data(self):
        """
        Confirma que o método Request.prepare() processa corretamente
        o atributo `json` e o coloca no corpo do PreparedRequest.
        Mata o mutante que anula o parâmetro json.
        """
        # 1. Configuração de um objeto Request com dados json
        json_data = {"key": "value", "another_key": [1, 2, 3]}
        req = Request(
            method="POST",
        ...
        # 3. Verificar (Assert)
        # O corpo do pedido preparado deve ser a string JSON
        # codificada dos dados originais.
